...
\begin{document}
    ...
    \subsection{Options}
    \Option{engine}\WithValues{auto,curl,wget,ariaII,axel}\AndDefault{auto}
...
\end{MacroCode}
    ...
    for all engines:
    \begin{macro*}{\__download_if_executable_test:TF}
    \begin{macro*}{\__download_if_executable_test:T}
    \begin{macro*}{\__download_if_executable_test:F}
    \begin{macro}{\__download_if_executable_test_p:}[1]
...
\begin{MacroCode}{package}
        \immediate\write18{wget~-q~--auto-file-renaming=false~#2}
    }{
\end{MacroCode}
    Again, when we have an optional argument we add the \texttt{-o}
    flag to specify the output file.
\begin{MacroCode}{package}
        \immediate\write18{wget~-q~--auto-file-renaming=false~-o~#1~#2}
    }
}
\end{MacroCode}
    \end{macro}
...
\end{MacroCode}
    ...
    \changes{1.1}{Use predicate instead of multiple if statements}
\begin{MacroCode}{package}
    \if_predicate:w{\__download_if_executable_test_p:n{wget} ||
                    \__download_if_executable_test_p:n{curl} ||
                    \__download_if_executable_test_p:n{aria2c} ||
                    \__download_if_executable_test_p:n{axel}}
        \prg_return_true:
    \else:
        \prg_return_false:
    \fi:
